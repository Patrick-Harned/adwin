    ...
    for i in buf.trim().split('\n') {
        ...
        if vec.len()as i32 > window_size{
            ...
            let vec2_avg = vec2.iter().sum::<i32>() as f32 / vec2.len() as f32;
                ...
                if (vec1_avg -vec2_avg).abs() > e_cut(vec.len() as i32, vec2.len() as i32) {
                    let out: String = "Found drift at ".to_owned() + &x.to_string() + " with first window average " + &vec1_avg.to_string() + " and second window average " + &vec2_avg.to_string() +  "\n";
